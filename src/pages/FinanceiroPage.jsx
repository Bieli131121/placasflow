...
export default function FinanceiroPage() {
  ...
  const abrirPagCli = (p) => {
    setEditId(p.id)
    setPagCliForm({ pagamento: p.pagamento || 'Aguardando', forma: p.forma || 'Pix' })
    setShowPagCliModal(true)
  }
  const salvarPagCli = async () => {
    await update(editId, pagCliForm)
    setShowPagCliModal(false)
    toast.success('Pagamento do cliente atualizado!')
  }
  const marcarPagoCli = async (id) => {
    await update(id, { pagamento: 'Pago' })
    toast.success('Marcado como recebido!')
  }

  const abrirPagFab = (p) => {
    setEditId(p.id)
    setPagFabForm({ pagFabStatus: p.pagFabStatus || 'A pagar', pagFabValor: p.pagFabValor || '', pagFabForma: p.pagFabForma || 'Pix' })
    setShowPagFabModal(true)
  }
  const salvarPagFab = async () => {
    await update(editId, pagFabForm)
    setShowPagFabModal(false)
    toast.success('Pagamento à fábrica atualizado!')
  }
  const marcarPagoFab = async (id) => {
    await update(id, { pagFabStatus: 'Pago' })
    toast.success('Pagamento à fábrica confirmado!')
  }
  ...
  return (
    <>
      ...
      <div className="stats">
        {resumeCards.map(c => (
          <div key={c.label} className="stat-card" style={{ borderTop: `2px solid ${c.color}` }}>
            ...
            <div className="stat-value" style={{ color: c.color, fontSize: '1.4rem' }}>{formatVal(c.value)}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid var(--border)', marginBottom: '1.5rem' }}>
        {[['clientes','👤 Recebimentos de Clientes'],['fabrica','🏭 Pagamentos à Fábrica']].map(([key, label]) => (
